...
- **`terraform.tfvars` / `config.tf` split.** Per-env values (`scaleway_project_id`, `scaleway_region`, `scaleway_zone`, `environment`, `base_domain`, `identity_domain`, `mgmt_domain`, `infra_email`, `compute_instance_types`, `compute_disk_sizes`, `private_network_subnet`) move into `terraform.tfvars` (gitignored — new `terraform.tfvars.example` ships as the template). Persistent infrastructure shape (`org_slug`, `org_name`, `compute_hosts` topology, `identity.tier_slots`, `apps` catalog, `bootstrap`, `base`, `custom_dns_records`, `manage_gateway_dns`) stays in `config.tf` (committable). The rule: anything that VARIES between staging and prod lives in tfvars; anything that doesn't lives in config.tf. App hostnames in config.tf interpolate against `${var.base_domain}` so the same config.tf serves every env. `compute_hosts` references `var.compute_instance_types.<key>` / `var.compute_disk_sizes.<key>` for per-host sizing. New `tfvar_value` helper in `_lib.sh` + `preflight.sh` mirrors the existing `config_value` helper but reads from terraform.tfvars.
- **`consumer-template/apps-manifest.yaml` schema heading: `tfvars:` → `config:`** (18 entries). Reflects that the values it describes go into `config.tf`, not `.tfvars` files. `scripts/check-manifest-coverage.sh` yq path updated to `.config.schema`.
- **ARCHITECTURE.md cutover-knobs section rewritten.** Dropped the three-mode table (greenfield / in-place / consumer-supplied source); replaced with a brief "how credentials get seeded" note pointing at the `lifecycle { ignore_changes = [data] }` safety net on every `scaleway_secret_version`.
- **README + docs sweep.** Stopped conflating "terraform variable" with ".tfvars file format." Repo-layout block now lists `config.tf.example` (persistent) + `terraform.tfvars.example` (per-env) + `backend.hcl.example` + `inventory.ini.example` (generated post-apply, kept as reference).

### Removed

- **`credentials_preserve` and `credentials_preserve_source` variables.** Every credential-generating bundle (outline, steward, vikunja, bentopdf, broadsheet, privacy-policy, nextcloud, decidim, jitsi, espocrm, n8n, backrest, grafana, wazuh manager, wazuh-agent), every shared module (`modules/authentik/oidc-app`, `modules/authentik/saml-app`, `modules/infrastructure/storage/postgres`, `modules/infrastructure/storage/postgres_database`), and `platform/identity/bootstrap` + `platform/base/terraform/{postgres,tem}.tf` drop both knobs. The three-mode cutover dance (greenfield / in-place legacy / consumer-supplied source) collapses to one mode: greenfield-with-`ignore_changes`-pinned-bag. Safe because every existing consumer is post-cutover (`credentials_preserve = false`) and every `scaleway_secret_version` carries `lifecycle { ignore_changes = [data] }`, so the bag content survives the removal regardless of what `random_*` resources now evaluate to. 65 files cleaned, 874 lines removed, 161 added.
- **`platform/apps/*/grafana_dashboards/` doc references to per-host backrest module addresses** (`module.backrest_mgmt`, `module.backrest_tools`, `module.backrest_gateway`). Replaced with the canonical v3.4.0+ `for inst in module.backrest : inst.authentik_provider_id` for-each pattern in ARCHITECTURE.md.
- **Dead doc references from the v3.4.0 architecture move.** README, ARCHITECTURE.md, `docker/runner/README.md`, bundle READMEs scrubbed of stale references to `module.backrest_mgmt`, `module.diun_mgmt`, `module.autoheal_apps`, `module.wazuh_agent_apps`, `module.loki`/`prometheus`/`grafana`/`wazuh` (now `module.core.<svc>`), watchtower, notifuse, the deleted `scripts/sabokit-runner.sh` wrapper. `apps-manifest.yaml`'s backrest entry rekeyed from `backrest_mgmt` to `backrest`. `gen_apps_yml.py` docstring updated to reference MULTI_INSTANCE_BUNDLES.

### Added

- **CI restart-policy guard workflow** at `.github/workflows/restart-policy-guard.yml`. On every PR + push to master, greps `platform/{apps,base,bootstrap,core}/**/*docker-compose*.j2`, fails the check if any `services:` block lacks `restart: unless-stopped` OR `restart: always`. Per-service opt-out via `#ci-restart-policy-warning-off` comment on the line directly above the service key (used today only by decidim's one-shot db-init job). Jinja-aware parser strips `{% ... %}` and `{{ ... }}` before YAML-parsing so templates survive. Posts an idempotent PR comment listing offenders.

### Fixed

- **Triple `host_services` output collision in `platform/base/terraform/outputs.tf`.** Merge regression from the v3.4.0 parallel host-services moves: each move (diun / autoheal / wazuh-agent) added its own `output "host_services" { ... }` block; never consolidated. Terraform refused to plan with "An output named `host_services` was already defined." Surfaced by the test fixture during the dead-config sweep. Now one block exposing all three services keyed by `<service> = { <host_key> = { enabled, ansible_vars, ansible_group, ... } }`.
- **`var.diun` sensitive flag blocking the `for_each` over `var.compute_hosts`.** `host_services.tf`'s diun module call uses `for_each = var.diun.enabled ? { for k, v in var.compute_hosts : k => v if !contains(var.diun.disabled_hosts, k) } : {}`. Terraform forbids sensitive values as `for_each` map arguments because the keys would leak into resource addresses. `var.diun` had `sensitive = true` defensively; dropped since its fields (image_tag, watch_schedule, n8n_webhook_url) carry no secret material.
- **Test fixture (`tests/local-validate/main.tf`) updated to v3.4.0 shape.** Source paths for diun / autoheal / wazuh-agent now point at `platform/base/host-services/<svc>/terraform`; module instance names dropped the `_mgmt` / `_apps` suffixes; added `tier_slots` to the identity module (required field).
- **`MANIFEST_ONLY_INPUTS` in `scripts/check-manifest-coverage.sh`** extended to cover 8 pre-existing n8n consumer-template-only inputs (`slack_channel_new_signups`, `slack_channel_admin_alerts`, `broadsheet_workspace_id`, `broadsheet_all_members_list_id`, `broadsheet_all_members_list_name`, `workflow_id_broadsheet_subscribe`, `workflow_id_espocrm_upsert`, `workflow_id_slack_invite_stub`). Validator now runs clean (0 errors, 205 operator-only warnings).
- **`modules/infrastructure/storage/postgres/main.tf` syntax debris** from the regex-driven credentials_preserve removal: a corrupted `locals { ... }` block and a malformed `output "database_credentials_secrets"` value expression. Rebuilt to the clean post-removal shape.

### Operator migration notes

- **Drop every `credentials_preserve` and `credentials_preserve_source` key from your `config.tf`.** After bumping to v3.5.0, terraform errors with "Unsupported argument" on any module block that passes them. Greenfield consumers and post-cutover consumers (everyone, per the v3.4 ship's "all migrations done" assertion) see zero resource churn — `random_*` resources stay in state, `scaleway_secret_version`'s `ignore_changes = [data]` keeps the bag contents pinned. Anyone who somehow still has `credentials_preserve = true` set: flip to `false`, apply, verify the bag is populated as expected, then bump to v3.5.0 and drop the key.
- **Move per-env values from `config.tf` into a new `terraform.tfvars`.** Copy `consumer-template/environments/_template/terraform.tfvars.example` to your env directory as `terraform.tfvars` (gitignored), populate `scaleway_project_id`, `base_domain`, `identity_domain`, `mgmt_domain`, `infra_email`, `environment`, `scaleway_region`, `scaleway_zone`, `compute_instance_types`, `compute_disk_sizes`, `private_network_subnet`. Strip those keys from your `config.tf`'s `locals.config` block. App hostnames in config.tf change from `"wiki.example.org"` to `"wiki.${var.base_domain}"` (or you can keep them hardcoded if you really want — the rule is per-env-varying things live in tfvars, but consumers can split however they want).
- **`compute_hosts` topology** stays in config.tf but `instance_type` / `disk_size` per host now reference `var.compute_instance_types.<key>` / `var.compute_disk_sizes.<key>`. Same shape, just parameterized.
- **`preflight.sh` and `_lib.sh` now `require_files` terraform.tfvars** in addition to config.tf + backend.hcl + inventory.ini. Missing terraform.tfvars = preflight fails fast with a clear error pointing at the .example template.
- **CI restart-policy guard will start failing PRs** that introduce compose templates without `restart: unless-stopped` (or `restart: always`) on every service block. Opt out per-service via `#ci-restart-policy-warning-off` comment on the line directly above the service key — for legitimate one-shot init jobs (already wired for decidim's db-init).
- **No new state migrations** beyond what v3.4.0 already shipped. `consumer-template/modules/stack/migrations.tf` carries the v3.4.0 `moved{}` blocks; v3.5.0 adds none.
