...
- **Move per-env values from `config.tf` into a new `terraform.tfvars`.** Copy `consumer-template/environments/_template/terraform.tfvars.example` to your env directory as `terraform.tfvars` (gitignored), populate `scaleway_project_id`, `base_domain`, `gateway_domain`, `mgmt_domain`, `infra_email`, `environment`, `scaleway_region`, `scaleway_zone`, `compute_instance_types`, `compute_disk_sizes`, `private_network_subnet`. Strip those keys from your `config.tf`'s `locals.config` block. App hostnames in config.tf change from `"wiki.example.org"` to `"wiki.${var.base_domain}"` (or you can keep them hardcoded if you really want — the rule is per-env-varying things live in tfvars, but consumers can split however they want).
