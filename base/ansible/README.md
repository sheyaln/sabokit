...
The bootstrap layer every consumer of `sabokit` runs on its hosts before deploying any app bundle. App bundles (`apps/<name>/ansible/`) `import_role` from here for Docker, Traefik labels, secret fetching, and the monitoring agent.
...
`base/ansible` is versioned together with the rest of the repo (see `ARCHITECTURE.md`'s "Versioning" section). Consumers pin via a repo-level tag. Breaking changes to a role's variable surface bump the major version of the entire blueprint.
