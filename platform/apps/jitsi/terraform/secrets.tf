...
locals {
  ...
  jwt_app_secret          = var.enabled ? (var.credentials_preserve ? local._preserved.JITSI_JWT_APP_SECRET : random_password.jwt_app_secret[0].result) : ""
  jicofo_auth_password    = var.enabled ? (var.credentials_preserve ? local._preserved.JITSI_JICOFO_AUTH_PASSWORD : random_password.jicofo_auth[0].result) : ""
...
resource "scaleway_secret_version" "app" {
  ...
  data = jsonencode({
    JITSI_JWT_APP_SECRET          = local.jwt_app_secret
    JITSI_JICOFO_AUTH_PASSWORD    = local.jicofo_auth_password
    JITSI_JVB_AUTH_PASSWORD       = local.jvb_auth_password
    JITSI_JIBRI_XMPP_PASSWORD     = local.jibri_xmpp_password
    JITSI_JIBRI_RECORDER_PASSWORD = local.jibri_recorder_password
