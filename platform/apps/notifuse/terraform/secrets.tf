# Notifuse encrypts every workspace secret with SECRET_KEY. Rotating it
...
# Once Notifuse boots and the root user exists, rotating here doesn't change
...
resource "scaleway_secret" "app" {
  ...
  description = "Notifuse application secrets (SECRET_KEY, root admin, OIDC, S3, SMTP from-address)."
...
resource "scaleway_secret_version" "app" {
  ...
  data = jsonencode({
    ...
    ROOT_ADMIN_PASSWORD = random_password.root_admin[0].result
