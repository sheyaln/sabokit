...
output "backup_plan" {
  description = "Backrest backup plan contribution. null when disabled or backup_enabled = false. Aggregated by consumer-template into backrest's backup_plans. `espocrm-data` holds /var/www/html (app code + uploads + custom extensions); RDB postgres has its own managed backups."
