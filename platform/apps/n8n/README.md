...
| `grafana-alert-router.json` | Generic Grafana alert webhook receiver on `/webhook/grafana-alerts`. Parses Grafana's standard alert webhook payload (`alerts[]` with labels/annotations/status/severity), fans out by `labels.severity` (`critical` / `warning` / `info`) to Slack + email. Point Grafana's webhook contact point at `https://<n8n hostname>/webhook/grafana-alerts`. Required env: `SLACK_WEBHOOK_INFRA_ALERTS` (warning + info), optional `SLACK_WEBHOOK_INFRA_CRITICAL` (falls back to the alerts webhook), `INFRA_EMAIL` + `SMTP_FROM` for the critical email path. When the grafana bundle has `jsm_api_key_secret_id` set, JSM is Grafana's *default* contact point — this workflow is secondary, used when you want slack/email fan-out alongside (or instead of) JSM. Wire it in as an additional contact point on Grafana's side. |
