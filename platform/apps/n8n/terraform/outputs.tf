...
output "backup_plan" {
  description = "Backrest backup plan contribution. null when disabled or backup_enabled = false. Aggregated by consumer-template into backrest's backup_plans. `n8n_data` is `/home/node/.n8n` (workflows, encrypted credentials, execution history)."
