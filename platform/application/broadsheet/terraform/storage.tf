...
resource "scaleway_iam_application" "storage" {
  ...
  description = "S3 access for Broadsheet files bucket (${var.base.scaleway.secrets_namespace})"
...
resource "scaleway_iam_policy" "storage" {
  ...
  description    = "Object Storage access for Broadsheet files bucket (${var.base.scaleway.secrets_namespace})"
...
resource "scaleway_iam_api_key" "storage" {
  ...
  application_id = scaleway_iam_application.storage[0].id
  description    = "API key for Broadsheet S3 files access"
}
