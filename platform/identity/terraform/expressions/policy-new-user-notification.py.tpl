...
# Sends email and webhook notifications when a new user signs up.
#
# Template variables (rendered by Terraform templatefile()):
#   - webhook_url:        URL for the lifecycle webhook (may be empty)
#   - domain:             base apps domain (e.g. example.org)
#   - organisation_name:  Organization display name
#   - target_groups_json: JSON list of group names that should receive the email
...
def send_webhook_notification(event_type, user_data):
    """POST the signup event to the configured webhook.

    If the webhook responds with JSON containing a correlation key
    ("correlation_id" / "thread_ts"), we store it on the user so the later
    activation notification can reference the same thread."""
    if not WEBHOOK_URL:
        ak_logger.warning("Webhook URL not configured")
        return None
    ...
    try:
        ...
        ak_logger.info(f"Sent {event_type} notification to webhook")
...
You can manage users at: https://${domain}/if/admin/#/identity/users
"""

# =========================================================================
# 1. SEND EMAIL TO ADMIN-TIER GROUPS (independent of webhook)
# =========================================================================
email_sent = False
try:
    recipients = set()
...
# =========================================================================
# 2. SEND WEBHOOK NOTIFICATION (independent of email)
# =========================================================================
webhook_sent = False
correlation_id = None
try:
    ...
    # If the webhook returned a correlation key, store it on the user so the
    # later activation notification can correlate (e.g. Slack thread reply).
    if webhook_response and isinstance(webhook_response, dict):
        correlation_id = (
            webhook_response.get("correlation_id")
            or webhook_response.get("thread_ts")
            or webhook_response.get("threadTs")
        )
        if correlation_id:
            user.attributes["signup_correlation_id"] = correlation_id
            ak_logger.info(f"Stored signup correlation id {correlation_id} for {user_email}")
except Exception as e:
    ak_logger.error(f"Failed to send webhook notification for {user_email}: {e}")
...
if email_sent or webhook_sent:
    ...
    user.save()
    ak_logger.info(f"Enrollment notifications sent for {user_email}: email={email_sent}, webhook={webhook_sent}")
